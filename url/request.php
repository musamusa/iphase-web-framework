 ...
 */
$GLOBALS['_IREQUEST'] = array();

/**
 * Set the available masks for cleaning variables
 */
define('IREQUEST_NOTRIM',	1);
define('IREQUEST_ALLOWRAW',	2);
define('IREQUEST_ALLOWHTML',4);

/**
 * request Class
 *
 * This class serves to provide the iphase Framework with a common interface to access
...
*/
	...
	public static function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
	{
		...
		if (isset($GLOBALS['_IREQUEST'][$name]['SET.'.$hash]) && ($GLOBALS['_IREQUEST'][$name]['SET.'.$hash] === true)) {
		...
		elseif (!isset($GLOBALS['_IREQUEST'][$name][$sig]))
		{
			if (isset($input[$name]) && $input[$name] !== null) {
				...
				$GLOBALS['_IREQUEST'][$name][$sig] = $var;
			}
			elseif ($default !== null) {
			...
			$var = $GLOBALS['_IREQUEST'][$name][$sig];
		}
	...
	{
		// Cast to string, in case IREQUEST_ALLOWRAW was specified for mask
	...
	public static function setVar($name, $value = null, $hash = 'method', $overwrite = true)
	{
		...
		$GLOBALS['_IREQUEST'][$name]['SET.'.$hash] = true;
		$GLOBALS['_IREQUEST'][$name]['SET.REQUEST'] = true;
	...
	static function _cleanArray(&$array, $globalise=false)
	{
		...
		foreach ($array as $key => $value)
		{
			...
			if ($failed) {
				iexit('Illegal variable <b>' . implode('</b> or <b>', $banned) . '</b> passed to script.');
			}
			if ($globalise) {
				$GLOBALS[$key] = $value;
			}
	...
	{
		$value = is_array($value) ? array_map(array('IREQuest', '_stripSlashesRecursive'), $value) : stripslashes($value);
		return $value;
	}
}
